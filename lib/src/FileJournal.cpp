...
static std::string generate_filename() {
    return std::string(DefaultLogName) + current_time_str() + std::string(DefaultLogExt);
}
...
void FileJournal::write(const Message &message) {
    if (message.message_level < journal_level) {
        return;
    }
    journal << current_time_str() << ' ' << LogLevel_to_string(message.message_level) << ' ' << message.message_text
            << std::endl;
}
